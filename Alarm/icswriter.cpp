...
int writetoics(std::vector<Datestruct> dates) {
	...
	if (outputfile.is_open()) {
		for (unsigned int it = 0; it < dates.size(); it++) {
			...
			if (dates[it].hour < 10) {
				datetimebefore += "0";
				datetimebefore += std::to_string(dates[it].hour);
			}
			...
			if (dates[it].hourafter < 10) {
				datetimeafter += "0";
				datetimeafter += std::to_string(dates[it].hourafter);
			}
			...
			if (dates[it].minute = 0)
				datetimebefore += "0000";
			else if (dates[it].minute < 10 && dates[it].minute != 0) { 
				datetimebefore += "0";
				datetimebefore += std::to_string(dates[it].minute) += "00"; }
			else { datetimebefore += "0000"; }
			
			
			if (dates[it].minuteafter = 0) {
				datetimeafter += "0000";
			}
			else if (dates[it].minuteafter < 10 && dates[it].minuteafter != 0) { 
				datetimeafter += "0";
				datetimeafter += std::to_string(dates[it].minuteafter);
				datetimeafter += "00";
			}
			else { datetimeafter += "0000"; }
