...
{
	result = sscanf(input.c_str(), formatter, dage, maaneder, aar, timer, minutter); //looks for first " then / then - then : and puts them into char arrays
...
{
	result = sscanf(input.c_str(), "%[^/]/%[^-]-%s%[^:]:%s", dage, maaneder, aar, timer, minutter); //looks for first " then / then - then : and puts them into char arrays
	Datestruct temp_date = {};
	temp_date.year = atoi(aar); //converts the char * to an integer.
	temp_date.month = atoi(maaneder);
	temp_date.day = atoi(dage);
